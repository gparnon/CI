    ...
    for tab in tabs:
        ...
        for val in tab_dict:
            if val['tab_name'] == tab:
                ...
                for i in graphlist:
                    hasnumentries = get_num_entries(i) #Get the number to determine which data to use from the optional number paramater (ie last 30)
...
#This function checks whether the number input has been entered and sets the variable hasnumentries
#to 1 if there is an entry in "x:y" form, 2 if it is in ":x", and 3 if it is in "x:"
def get_num_entries(i):
    try: 
        number = i['number']
